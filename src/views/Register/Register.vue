...
const errorType = {
   incorrectCredentials: 'Sorry, invalid credentials. Please fix and try again!',
   existingEmail: 'Sorry, that email has already been registered. Try another email!',
};
...
const validateFields = () => {
   ...
   error.confirmPassword = user.confirmPassword !== user.password || !user.confirmPassword;
...
const validateExistingEmail = async () => {
   return fetch('http://localhost:3001/users')
      ...
      .then(data => {
         const existingEmail = data.find((userList: UserTypes) => userList.email === user.email);
         if (existingEmail) {
            error.email = true;
      ...
      <section class="flex justify-center items-center h-[100%]">
         <div class="flex justify-center h-[100%] items-center flex-col w-full">
            ...
            >
               ...
               >
               <span v-if="!!errorText" class="text-black font-medium">{{ errorText }}</span>
